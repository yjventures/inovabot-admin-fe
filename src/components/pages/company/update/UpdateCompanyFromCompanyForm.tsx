...
export default function UpdateCompanyFromCompanyForm() {
  const push = usePush()

  const { data, isSuccess: isFetchSuccess } = useGetCompanyQuery(getCompanyId())
  ...
  const onSubmit = (data: Partial<ICompany>) => {
    updateCompany({ id: getCompanyId(), body: data })
  }
